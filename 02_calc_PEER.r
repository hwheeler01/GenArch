...
write.table(factors,file=rna.dir %&% "All_tissues.GTEx_Data_2014-06-13.protein-coding." %&% Nk %&% ".PEER.factors." %&% date %&% ".txt", quote=F)
...
write.table(finalinfo,file=rna.dir %&% "GTEx_Analysis_2014-06-13.RNA-seq.IDinfo.PCs.PFs",sep="\t",quote=F,row.names=F)

weights = PEER_getW(model)
precision = PEER_getAlpha(model)
residuals = PEER_getResiduals(model)

pdf(file=rna.dir %&% "All_tissues.GTEx_Data_2014-06-13.protein-coding." %&% Nk %&% ".PEER.factors.plotmodel." %&% date %&% ".pdf")
