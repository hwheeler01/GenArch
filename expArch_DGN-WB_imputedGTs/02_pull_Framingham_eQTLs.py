...
outfileAll = open('test.Framingham_eqtl-gene_p0.0001_hapmapSnpsCEU.rsIDlist','w')
outfileFDR = open('test.Framingham_eqtl-gene_fdr0.05_hapmapSnpsCEU.rsIDlist','w')

haplist = open(hapmap).read().splitlines()  ##put each line of file in list with no newline

f = gzip.open(framing,'rb')
for line in f:
    linelist = line.split(',')
    snp = linelist[3]
    if snp == 'Rs_ID':
