...
#args <- c('22','C','Skin-SunExposed(Lowerleg)')
...
ngroups = 4 ##split chr into ngroups, edit run*.sh files to match groups
...
for(i in 1:length(explist)){
  ...
  if(dim(cisgenos)[1] > 0){
    ...
    write.table(annotfile, file=out.dir %&% "tmpTW.annot." %&% chrom %&% ".s." %&% whichlist, quote=F, row.names=F, 
	col.names=F, sep=",")
    write.table(genofile, file=out.dir %&% "tmpTW.geno." %&% chrom %&% ".s." %&% whichlist, quote=F, row.names=F, 
	col.names=F, sep=",")
    write.table(phenofile, file=out.dir %&% "tmpTW.pheno." %&% chrom %&% ".s." %&% whichlist, quote=F, row.names=F,
	col.names=F, sep=",")

    runBSLMM <- "gemma -g " %&% out.dir %&% "tmpTW.geno." %&% chrom %&% ".s." %&% whichlist %&% " -p " %&% out.dir %&% 
	"tmpTW.pheno." %&% chrom %&% ".s." %&% whichlist %&% " -a " %&% out.dir %&% "tmpTW.annot." %&% chrom %&% ".s." %&% 
	whichlist %&% " -bslmm 1 -seed 12345 -s 100000 -o tmpTW." %&% chrom %&% ".s." %&% whichlist
    system(runBSLMM)

    hyp <- read.table(bslmm.out.dir %&% "output/tmpTW." %&% chrom %&% ".s." %&% whichlist %&% ".hyp.txt",header=T)
