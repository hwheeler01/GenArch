...
```{r jointH2, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="DGN-WB joint heritability. Local h^2 is estimated with SNPs within 1 Mb of each gene. Global h^2 is estimated with SNPs that are eQTLs in the Framingham Heart Study on other chromosomes (FDR < 0.05).",fig.width=6,fig.height=9}
...
b<-ggplot(data,aes(x=1:nrow(data),y=h2,ymin=ymin, ymax=ymax) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("Local h"^2)) + xlab(expression("genes ordered by Local h"^2))+coord_cartesian(ylim=c(0,1))
...
data <- data[complete.cases(data),]
c<-ggplot(data,aes(x=1:nrow(data),y=h2,ymin=ymin, ymax=ymax) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("Global h"^2)) + xlab(expression("genes ordered by Global h"^2))
