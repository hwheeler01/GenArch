...
% Please keep the abstract below 300 words (currently 296 words, Nat. Comm: 150 words)
\section*{Abstract}
...
\section*{Author Summary} %currently xxx words
...
We call this approach orthogonal tissue decomposition (OTD) because the cross-tissue and tissue-specific components are assumed to be independent in the model. The decomposition is applied at the expression trait level so that the downstream genetic regulation analysis is performed separately for each derived trait, cross-tissue and tissue-specific expression, which greatly reduces computational burden. We applied OTD to the observed expression data and compared the results to OTD of simulated expression data. The median Pearson correlation between observed and simulated OTD for the cross-tissue component was 0.54 [95\% CI: 0.33-0.77] and for tissue-specific component was 0.75 [0.46-0.88], demonstrating the robustness of our model (\nameref{S5_Fig}). For the OTD derived cross-tissue and 40 tissue-specific expression phenotypes, we computed the local heritability and generated prediction models.
...
Cross-tissue predictive performance exceeded that of both tissue-specific and whole tissue expression as indicated by higher cross-validated R\textsuperscript{2} (\nameref{S2_Fig}, \nameref{S9_Fig}). Like whole tissue expression, cross-tissue and tissue-specific expression showed higher predictive performance when using more sparse models. In other words elastic-net models with \(\alpha \geq 0.5\) predicted better than the ones with \(\alpha=0.05\) (\nameref{S10_Fig}). 
...
To test the robustness of our method, we derived simulated expression levels from the observed OTD components. The simulated expression phenotype is the sum of the observed OTD cross-tissue and tissue-specific components plus an error term derived by randomly sampling a normal distribution with standard deviation equal to the variance of each gene. \hl{[[Haky, put previous sentence in equation form after checking the above equations?]]} We performed OTD on this simulated phenotype and compared the cross-tissue and tissue-specific expression levels for each gene to the observed levels using Pearson correlation.
...
\noindent {\bf DGN local heritability estimation comparison between LDAK and GCTA.} The Pearson correlation between GCTA and the LD-adjusted kinship local h$^2$ estimates of gene expression is 0.96. In red is the identity line and in blue is the best fit linear regression line, which indicates that on average, the GCTA estimates are slightly higher than the LDAK estimates.
...
\noindent {\bf Correlation of orthogonal tissue decomposition (OTD) applied to simulated v. observed expression.} The simulated phenotype is the sum of the observed cross-tissue and tissue-specific components plus an error term derived by randomly sampling a normal distribution with standard deviation equal to the variance of each gene. {\bf (A)} The example gene \textit{SLTM} has a cross-tissue correlation near the median of all genes (median R = 0.54) and significant correlations in all tissue-specific components. {\bf(B)} Distribution of the correlation between simulated v. observed OTD expression across genes.
...
\noindent {\bf Continuous distribution of heritability of the cross-tissue component vs. a measure of uniformity of genetic regulation across tissues.}
