...
###calc % nonzero for each tissue TISSUE-SPECIFIC
### ADD to plot legend
b<-gTS_h2_se %>% select(ensid,Tissue,nonzeroCI) %>% spread(Tissue,nonzeroCI)
for(i in 2:11){
  tis<-colnames(b)[i]
  cat("\n\n---",tis,"---\n")
  print(table(b[,i]))
  print(round(table(b[,i])/sum(table(b[,i])),3))
}
###calc mean h2 for each tissue
b<-gTS_h2_se %>% select(ensid,Tissue,h2) %>% spread(Tissue,h2)
for(i in 2:11){
  tis<-colnames(b)[i]
  cat(tis,"mean h2:",mean(b[,i]),"\n")
}
```
