...
a<-ggplot(g_cten, aes(x = R2 , y = g_cten[,3], group=alpha, color=alpha)) + geom_point(show_guide = TRUE) + xlab(expression(paste("alpha R"^2))) + ylab(expression(paste("lasso (",alpha,"=1) R"^2))) + geom_smooth(method = "lm")+geom_abline(intercept=0, slope=1,color='black')+ coord_cartesian(ylim=c(-0.05,1.05),xlim=c(-0.05,1.05))+ ggtitle("GTEx cross-tissue (" %&% ngenesall %&% " genes)")+theme_gray(18)
a
