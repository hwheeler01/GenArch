...
{[}9,19,20{]}. Although more recently, studies with a few other tissues have been published, a comprehensive coverage of human tissues was not available until the GTEx project was started. Genotype-Tissue Expression (GTEx) Project aims to
...
shared across tissues {[}21{]}. Here we use a much larger set of 8xxx samples corresponding to up to xxx individuals from the 2015 release. 

One of the findings of this comprehensive analysis was that a large portion of the local regulation of expression traits is shared across multiple tissues. This was corroborated by the fact that our prediction model based on whole blood showed good prediction across the 9 tissues with relatively large sample sizes {[}13{]}. 

This shared regulation implies that there is much to be learned from large sample studies of easily accessible tissues. Still a portion of the regulation seems to be tissue dependent.
...
Table 1 summarizes the heritability estimate results across all tissues. It shows the mean h2 (\hl{this should be computed allowing h2 estimates to be negative to avoid bias in the meanh2}), mean se of h2 (local and distal), proportion of local (mean(local)/(mean(local)+mean(distal))), proportion of significant h2, number of significant genes, and number of expressed genes (definition? more than xx individuals with xx RPKM).

\hl{Relative contribution of local and distal variation to h2.}
...
Given this limitation, we focus on local regulation for the remainder of the paper.
