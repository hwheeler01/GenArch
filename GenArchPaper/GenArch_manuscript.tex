...
Im\textsuperscript{1}}
  ...
  \postdate{\par}
  \date{2015-06-05 14:03:19 \textsuperscript{1}Department of Medicine,
...
are also polygenic or have simpler genetic architectures and how
expression architectures vary across genes {[}7{]}. most human to this
point local and in blood. cite Kruglyak review, Price PGen 11, Wright
NatGen 14
...
Depression Genes and Networks (DGN) cohort (n=922) {[}8{]} using a
mixed-effects model (see Methods) and calculated variances using
restricted maximum likelihood as implemented in GCTA {[}9{]}. We fit a
...
(n=5257, FDR \textless{} 0.05) {[}10{]}. The mean local
...
We performed 10-fold cross-validation using the elastic net {[}11{]} to
...
individuals from the Depression Genes and Networks (DGN) cohort {[}8{]},
...
the \emph{trans}-eQTL analysis in Battle et al. {[}8{]} and downloaded
...
Imputation-Server
(\url{https://imputationserver.sph.umich.edu/start.html}) {[}12,13{]}
...
{[}14{]} data release on 2014-06-13. Of the individuals with gene
...
GENCODE {[}15{]} version 12 gene annotation. We calculated the
...
maximum likelihood (REML) using GCTA software {[}9{]}. For heritability
...
intrinsic growth phenotype {[}16{]}. We applied OTD to GTEx Project
{[}14{]} data and decomposed the expression of each gene into
...
levels, we used the \texttt{lmer} function in the R {[}17{]} package
\texttt{lme4} {[}18,19{]} to fit the following mixed-effects model:
...
Protein-coding genes (defined by GENCODE {[}15{]} version 18) with a
...
across all tissues using the R package \texttt{PEER} {[}20{]} to control
...
We applied the elastic net {[}11{]} to model the effect of local genetic
...
\texttt{glmnet} {[}21,22{]} to perform 10-fold cross-validation of the
...
{[}21,22{]}. This tuning parameter \(\lambda\) controls the overall
strength of the penalty.

The elastic net penalty is controlled by mixing parameter \(\alpha\),
which spans LASSO (\(\alpha=1\), the default) {[}23{]} at one extreme
and ridge regression (\(\alpha=0\)) {[}24{]} at the other. The ridge
...
\midrule
\endhead
(Intercept) & -0.13 & 0.1 & -1.29 & 0.2\tabularnewline
x & 2.04 & 0.1 & 20.73 & 0.0\tabularnewline
\bottomrule
\end{longtable}
...
8. Battle A, Mostafavi S, Zhu X, Potash JB, Weissman MM, McCormick C, et
...
9. Yang J, Lee SH, Goddard ME, Visscher PM. GCTA: A tool for genome-wide
...
10. Zhang X, Joehanes R, Chen BH, Huan T, Ying S, Munson PJ, et al.
...
11. Zou H, Hastie T. Regularization and variable selection via the
...
12. Howie B, Fuchsberger C, Stephens M, Marchini J, Abecasis GR. Fast
...
13. Fuchsberger C, Abecasis GR, Hinds DA. Minimac2: Faster genotype
...
14. Ardlie KG, Deluca DS, Segre AV, Sullivan TJ, Young TR, Gelfand ET,
...
15. Harrow J, Frankish A, Gonzalez JM, Tapanari E, Diekhans M,
...
16. Im HK, Gamazon ER, Stark AL, Huang RS, Cox NJ, Dolan ME. Mixed
...
18. Bates D, Maechler M, Bolker B, Walker S. lme4: Linear mixed-effects
models using eigen and s4 {[}Internet{]}. 2014. Available:
\url{http://CRAN.R-project.org/package=lme4}

19. Bates D, Maechler M, Bolker BM, Walker S. lme4: Linear mixed-effects
models using eigen and s4 {[}Internet{]}. 2014. Available:
\url{http://arxiv.org/abs/1406.5823}

20. Stegle O, Parts L, Piipari M, Winn J, Durbin R. Using probabilistic
