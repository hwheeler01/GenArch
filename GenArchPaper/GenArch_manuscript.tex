...
Im\textsuperscript{1}}
  ...
  \postdate{\par}
  \date{2015-06-08 15:17:58 \textsuperscript{1}Department of Medicine,
...
{[}16{]} while highly polygenic traits should be better predicted with
ridge regression {[}17{]} or similarly polygenic models.
...
gene regulation, the genetic architecture is sparse rather than
...
estimates due to the borrowing of information across all samples. We
also found evidence that genes with large tissue-specific heritability
are enriched in common complex disease genes discovered via GWAS.
...
restricted maximum likelihood as implemented in GCTA {[}18{]}. We fit a
...
(n=5257, FDR \textless{} 0.05) {[}19{]}. The mean local
...
We performed 10-fold cross-validation using the elastic net {[}20{]} to
...
Imputation-Server
(\url{https://imputationserver.sph.umich.edu/start.html}) {[}21,22{]}
...
GENCODE {[}23{]} version 12 gene annotation. We calculated the
...
maximum likelihood (REML) using GCTA software {[}18{]}. For heritability
...
intrinsic growth phenotype {[}24{]}. We applied OTD to GTEx Project
...
levels, we used the \texttt{lmer} function in the R {[}25{]} package
\texttt{lme4} {[}26,27{]} to fit the following mixed-effects model:
...
Protein-coding genes (defined by GENCODE {[}23{]} version 18) with a
...
across all tissues using the R package \texttt{PEER} {[}28{]} to control
...
We applied the elastic net {[}20{]} to model the effect of local genetic
...
which spans LASSO (\(\alpha=1\), the default) {[}16{]} at one extreme
and ridge regression (\(\alpha=0\)) {[}17{]} at the other. The ridge
...
\midrule
\endhead
(Intercept) & -0.06 & 0.1 & -0.60 & 0.55\tabularnewline
x & 2.12 & 0.1 & 22.09 & 0.00\tabularnewline
\bottomrule
\end{longtable}
...
16. Regression shrinkage and selection via the lasso on jSTOR
{[}Internet{]}. \url{http://www.jstor.org/stable/2346178}; 2015.
Available: \url{http://www.jstor.org/stable/2346178}

17. Hoerl AE, Kennard RW. Ridge regression: Applications to
...
18. Yang J, Lee SH, Goddard ME, Visscher PM. GCTA: A tool for
...
19. Zhang X, Joehanes R, Chen BH, Huan T, Ying S, Munson PJ, et al.
...
21. Howie B, Fuchsberger C, Stephens M, Marchini J, Abecasis GR. Fast
...
22. Fuchsberger C, Abecasis GR, Hinds DA. Minimac2: Faster genotype
...
23. Harrow J, Frankish A, Gonzalez JM, Tapanari E, Diekhans M,
...
24. Im HK, Gamazon ER, Stark AL, Huang RS, Cox NJ, Dolan ME. Mixed
...
26. Bates D, Maechler M, Bolker B, Walker S. lme4: Linear mixed-effects
models using eigen and s4 {[}Internet{]}. 2014. Available:
\url{http://CRAN.R-project.org/package=lme4}

27. Bates D, Maechler M, Bolker BM, Walker S. lme4: Linear mixed-effects
models using eigen and s4 {[}Internet{]}. 2014. Available:
\url{http://arxiv.org/abs/1406.5823}

28. Stegle O, Parts L, Piipari M, Winn J, Durbin R. Using probabilistic
