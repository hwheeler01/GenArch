...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Heather E. Wheeler1,2, GTEx Consortium, Kaanan P. Shah3, Nicholas Knoblauch4, Nancy J. Cox5, Dan L. Nicolae3, Hae Kyung Im3},
            pdftitle={Genetic architecture of gene expression regulation across tissues and implications for prediction},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
Im\textsuperscript{3}}
  ...
  \postdate{\par}
  \date{2015-10-29 16:14:21 \textsuperscript{1}Department of Biology and
...
\(\alpha=0.5\). Howeer, a top eQTL only model did not do as well as the
...
reliably estimated at current sample sizes (although strong functional
priors reduce error). We also show that for local gene regulation, the
genetic architecture is mostly sparse rather than polygenic. We found
that estimates of the local heritability of cross-tissue gene expression
generated with our OTD model have larger magnitude and improved standard
errors compared to single tissue estimates due to the borrowing of
information across all samples. Unlike previous array-based expression
studies, we do not observe heritability estimate dependence on
expression level. Comparing our OTD results to a previously performed
joint multi-tissue eQTL analysis method {[}22{]}, we show that genes
with high cross-tissue heritability are more likely to have cross-tissue
eQTLs, confirming that OTD is capturing the cross-tissue component of
gene expression. Cross-validated predictors of cross-tissue and
tissue-specific expression built here have been added to the PrediXcan
database for future investigations of complex traits.
...
wright's h2 estimates b) RNAseq h2 vs RPKM in gtex{]}{]}
...
optimal for \(\alpha=0.5\) - \(\alpha=1\) in the DGN cohort (Fig 4). An
\(\alpha=0.05\) was also clearly suboptimal for gene expression
...
smaller than the tissue-specific estimates (Fig 9). The percentage of
GCTA h\textsuperscript{2} estimates with positive CIs was much larger
for cross-tissue expression (17.3\%) than the tissue-specific
expressions (all less than 3\%, Fig 10). Similarly, the percentage of
BSLMM PVE estimates with a lower credible set greater than 0.01 was 49\%
for cross-tissue expression, but ranged from 24-27\% for tissue-specific
expression (Fig 11).
...
95\% of heritable genes and that distal architecture cannot be
accurately assessed at current sample sizes (n \textless{} 1000).
{[}{[}compare mean h2's across studies{]}{]} Indeed, while the
heritability of gene expression attributable to local genetic variation
has been estimated accurately, large standard errors have prevented
accurate estimation of the contribution of distal genetic variation to
gene expression variation {[}11,12{]}.
...
{[}13{]}. The tissue availability is unbalanced because of the
...
models such as LASSO (Least Absolute Shrinkage and Selection Operator)
{[}14{]} while highly polygenic traits should be better predicted with
ridge regression or similarly polygenic models {[}15--17{]}. Using the
hybrid approach of BSLMM (Bayesian Sparse Linear Mixed Model) {[}18{]},
