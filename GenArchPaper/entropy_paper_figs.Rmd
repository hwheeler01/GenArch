...
for(tis in tislist){
  ...
  p <- ggplot(newdata,aes(x=h2,y=entropy))+geom_point(alpha=0.3)+stat_smooth(fill='pink',color='red')+xlab(expression(paste("Cross-Tissue ", "h"^{2}))) + ylab("Entropy")
  cors <- data.frame(cor=signif(cor(newdata$h2, newdata$entropy), 2))
  p1 <- p + geom_text(data=cors, aes(label=paste("R = ", cor, sep="")), x=Inf, y=-Inf, hjust=1.2, vjust=-1, size=5)
...
for(tis in tislist){
  ...
  p <- ggplot(newdata,aes(x=pve,y=entropy))+geom_point(alpha=0.3)+stat_smooth(fill='pink',color='red')+xlab("Cross-Tissue PVE") + ylab("Entropy")
