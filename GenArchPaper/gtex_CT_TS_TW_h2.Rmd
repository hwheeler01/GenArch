...
p<-ggplot(ts,aes(x=place,y=loc.jt.h2,ymin=ymin,ymax=ymax,color=nonzeroCI) ) + facet_wrap(~tissue,ncol=3) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by joint local h"^2))+theme_bw() + coord_cartesian(ylim=c(0,1)) + ggtitle("TW")
...
##rm cross-tissue from plot
ts <- filter(ts,tissue!="cross-tissue")
p<-ggplot(ts,aes(x=place,y=glo.jt.h2,ymin=ymin,ymax=ymax,color=nonzeroCI) ) + facet_wrap(~tissue,ncol=3) + geom_pointrange(col='gray')+geom_point()+ylab(expression("global h"^2)) + xlab(expression("genes ordered by joint global h"^2))+theme_bw() + coord_cartesian(ylim=c(0,1)) + ggtitle("TW")
...
for(i in 2:10){
  tis<-colnames(a)[i]
  meanh2 <- signif(mean(a[,i]),3)
  maxh2 <- signif(max(a[,i]),3)
  cat(tis,"mean h2:",meanh2,"max h2:",maxh2,"\n")
  h2vec <- c(h2vec,meanh2)
}
