...
pest <-  dgn %>%  mutate(local.q=p.adjust(local.p,method="BH"))  %>% arrange(local.h2) %>% mutate(Qlt05=local.q<0.1)
...
pest <-  dgn %>% mutate(glo.jt.P=2*pnorm(-abs(glo.jt.h2/glo.jt.se))) %>%  mutate(glo.jt.Q=p.adjust(glo.jt.P,method="BH"))   %>% arrange(glo.jt.h2) %>% mutate(Qlt05=glo.jt.Q<0.1) 
