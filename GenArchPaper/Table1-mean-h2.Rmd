...
meanh2 <- signif(mean(dgn$loc.jt.h2,na.rm=TRUE),3)
meanse <- signif(mean(dgn$loc.jt.se,na.rm=TRUE),3)
ciest <-  dgn %>% mutate(ymin = pmax(0, loc.jt.h2 - 2 * loc.jt.se), ymax = pmin(1, loc.jt.h2 + 2 * loc.jt.se)) %>% arrange(loc.jt.h2) %>% mutate(nonzeroCI=ymin>0,place=1:length(dgn$tissue)) 
