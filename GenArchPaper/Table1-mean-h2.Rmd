...
table1 <- matrix(0,nrow=length(tislist)+2,ncol=7)
n <- dgn$N[1]
numexpgenes <- dim(dgn)[1] 
meanh2 <- sprintf("%.3f",mean(dgn$loc.jt.h2,na.rm=TRUE))
meanse <- sprintf("%.3f",mean(dgn$loc.jt.se,na.rm=TRUE))
pest <-  dgn %>% mutate(loc.jt.P = 2*pnorm(-abs(loc.jt.h2/loc.jt.se))) %>% mutate(loc.jt.P=ifelse(is.na(loc.jt.P),1,loc.jt.P)) %>% arrange(loc.jt.h2) %>% mutate(Plt05=loc.jt.P<0.05) 
propsig <- sprintf("%.1f",table(pest$Plt05)[2]/sum(table(pest$Plt05,useNA="i"))*100)
numsig <- table(pest$Plt05)[2]
table1[1,] <- c("DGN Whole Blood",n,meanh2,meanse,propsig,numsig,numexpgenes)
...
meanh2 <- sprintf("%.3f",mean(ct$loc.jt.h2,na.rm=TRUE))
meanse <- sprintf("%.3f",mean(ct$loc.jt.se,na.rm=TRUE))
pest <-  ct %>% mutate(local.p=ifelse(is.na(local.p),1,local.p)) %>% arrange(local.h2) %>% mutate(Plt05=local.p<0.05)
propsig <- sprintf("%.1f",table(pest$Plt05)[2]/sum(table(pest$Plt05,useNA="i"))*100)
numsig <- table(pest$Plt05)[2]
table1[2,] <- c("Cross-tissue",n,meanh2,meanse,propsig,numsig,numexpgenes)

##calc mean global for DGN
signif(mean(dgn$loc.jt.h2,na.rm=TRUE),3)
signif(mean(dgn$loc.jt.se,na.rm=TRUE),3)
signif(mean(dgn$glo.jt.h2,na.rm=TRUE),3)
signif(mean(dgn$glo.jt.se,na.rm=TRUE),3)
##prop loc
...
for(i in 1:length(tislist)){
  ...
  meanse <- sprintf("%.3f",mean(data$local.se,na.rm=TRUE))
  ...
  tableinfo <- c(tisspace,n,meanh2,meanse,propsig,numsig,numexpgenes)
  table1[i+2,] <- tableinfo
}
colnames(table1)=c("tissue","n","avg h2","avg SE","% P<0.05","num P<0.05","num expressed")
#table1
