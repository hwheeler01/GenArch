...
pest <-  dgn %>% mutate(loc.jt.P=2*pnorm(-abs(loc.jt.h2/loc.jt.se))) %>% mutate(loc.jt.P=ifelse(is.na(loc.jt.P),1,loc.jt.P)) %>% mutate(loc.jt.Q=p.adjust(loc.jt.P,method="BH"))  %>% arrange(loc.jt.h2) %>% mutate(Qlt05=loc.jt.Q<0.1)
...
seperm <- setable$se[2]
meanandse <- meanh2 %&% " (" %&% seperm %&% ")"
pest <-  ct %>% mutate(local.p=ifelse(is.na(local.p),1,local.p)) %>% mutate(local.q=p.adjust(local.p,method="BH")) %>% arrange(local.h2) %>% mutate(Qlt05=local.q<0.1)
propsig <- sprintf("%.1f",table(pest$Qlt05)[2]/sum(table(pest$Qlt05,useNA="i"))*100)
numsig <- table(pest$Qlt05)[2]
...
pest <-  dgn %>% mutate(glo.jt.P=2*pnorm(-abs(glo.jt.h2/glo.jt.se))) %>% mutate(glo.jt.P=ifelse(is.na(glo.jt.P),1,glo.jt.P)) %>% mutate(glo.jt.Q=p.adjust(glo.jt.P,method="BH"))   %>% arrange(glo.jt.h2) %>% mutate(Qlt05=glo.jt.Q<0.1) 
...
for(i in 1:length(tislist)){
  ...
  pest <-  data %>% mutate(local.p=ifelse(is.na(local.p),1,local.p)) %>% mutate(local.q=p.adjust(local.p,method="BH")) %>% arrange(local.h2) %>% mutate(Qlt05=local.q<0.1) 
  propsig <- sprintf("%.1f",table(pest$Qlt05)[2]/sum(table(pest$Qlt05,useNA="i"))*100)
  numsig <- table(pest$Qlt05)[2]
...
colnames(table1)=c("tissue","n","mean h2 (SE)","% q-val<0.1","num q-val<0.1","num expressed")
#table1
