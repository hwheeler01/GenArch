...
groupdata <- dplyr::mutate(newdata, group=ifelse(entropy<1,0,1))
groupdata <- dplyr::mutate(newdata, group=ifelse(entropy>2,2,groupdata$group))
colnames(groupdata)[3] <- "h2"
p <- ggplot(groupdata,aes(y=h2,x=factor(group,labels=c("0-1","1-2","2-3"))))+geom_boxplot()+ylab(expression(paste("Cross-Tissue ", "h"^{2}))) + xlab("Uniformity")
cors <- data.frame(cor=signif(cor(groupdata$h2, groupdata$group), 2))
p1 <- p + geom_text(data=cors, aes(label=paste("R = ", cor, sep="")), x=-Inf, y=Inf, hjust=-0.5, vjust=2, size=5)
print(cor.test(groupdata$h2, groupdata$group)$p.value)
...
groupdata <- dplyr::mutate(newdata, group=ifelse(entropy<1,0,1))
groupdata <- dplyr::mutate(newdata, group=ifelse(entropy>2,2,groupdata$group))
colnames(groupdata)[3] <- "pve"
p <- ggplot(groupdata,aes(y=pve,x=factor(group,labels=c("0-1","1-2","2-3"))))+geom_boxplot()+ylab(expression(paste("Cross-Tissue ", "PVE"))) + xlab("Uniformity")
cors <- data.frame(cor=signif(cor(groupdata$pve, groupdata$group, use="p"), 2))
p1 <- p + geom_text(data=cors, aes(label=paste("R = ", cor, sep="")), x=-Inf, y=Inf, hjust=-0.5, vjust=2, size=5)
print(cor.test(groupdata$pve, groupdata$entropy)$p.value)
