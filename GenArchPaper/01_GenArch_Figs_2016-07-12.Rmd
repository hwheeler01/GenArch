...
local <- fdrother %>% select(loc.jt.h2,loc.jt.se,loc.jt.P,locPlt05,ensid)%>% arrange(loc.jt.h2) 
...
names(local) = c('h2','se','jt.P','Plt05','ensid')
local <- local %>% mutate(ymin =  h2 - 2 * se, ymax =  h2 + 2 * se )
cigt0 <- local$ymin>0
table(local$jt.P < 0.05,useNA='i')
sum(table(local$jt.P < 0.05,useNA='i'))
ptrue <-round(table(local$jt.P < 0.05,useNA='i')/sum(table(local$jt.P < 0.05,useNA='i')),3)*100
ptrue
meanh2<-round(mean(local$h2),3)
meanh2
meanse <- round(mean(local$se),3)
meanse
local <- mutate(local,P=factor(Plt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(local))
my_grob2 = grobTree(textGrob(substitute(paste("% P < 0.05: ", m),list(m=ptrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))
#my_grob3 = grobTree(textGrob(substitute(paste("mean h^2: ", m),list(m=meanh2)), x=0.05,  y=0.60, hjust=0,gp=gpar(fontsize=14)))

b<-ggplot(local,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=P) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by local h"^2))+theme_bw()+  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+ annotation_custom(my_grob2) 

global <- fdrother %>% select(gene,glo.jt.h2,glo.jt.se,glo.jt.P,gloPlt05,ensid) %>% arrange(glo.jt.h2) %>% mutate(glo.jt.h2=ifelse(is.na(glo.jt.h2),0,glo.jt.h2), glo.jt.se=ifelse(is.na(glo.jt.se),base::sample(glo.jt.se[is.na(glo.jt.se)==FALSE][1:1000],size=length(glo.jt.se[is.na(glo.jt.se)==TRUE]),replace=TRUE),glo.jt.se))%>% arrange(glo.jt.h2)  
names(global) = c('gene','h2','se','jt.P','Plt05','ensid')
global <- global %>% mutate(ymin =  h2 - 2 * se, ymax =  h2 + 2 * se)
table(global$jt.P < 0.05,useNA='i')
sum(table(global$jt.P < 0.05,useNA='i'))
ptrue <-signif(table(global$jt.P < 0.05,useNA='i')/sum(table(global$jt.P < 0.05,useNA='i')),3)*100
ptrue
meanh2<-round(mean(global$h2),3)
meanh2
meanse <- round(mean(global$se),3)
meanse
global <- mutate(global,P=factor(Plt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(global))

glopriorlist <- dplyr::filter(global,Plt05==TRUE) %>% dplyr::select(gene)

my_grob2 = grobTree(textGrob(substitute(paste("% P < 0.05: ", m),list(m=ptrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

c<-ggplot(global,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=P) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + 
...
localall <- other %>% select(loc.jt.h2,loc.jt.se,loc.jt.P,locPlt05,ensid) %>% arrange(loc.jt.h2) %>% 
...
names(localall) = c('h2','se','jt.P','Plt05','ensid')
localall <- localall %>% mutate(ymin =  h2 - 2 * se, ymax =  h2 + 2 * se )
table(localall$jt.P < 0.05,useNA='i')
sum(table(localall$jt.P < 0.05,useNA='i'))
ptrue <-round(table(localall$jt.P < 0.05,useNA='i')/sum(table(localall$jt.P < 0.05,useNA='i')),3)*100
ptrue
meanh2<-round(mean(localall$h2),3)
meanh2
meanse <- round(mean(localall$se),3)
meanse
localall <- mutate(localall,P=factor(Plt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(localall))
my_grob2 = grobTree(textGrob(substitute(paste("% P < 0.05: ", m),list(m=ptrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

bother<-ggplot(localall,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=P) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + 
  xlab(expression("genes ordered by local h"^2))+theme_bw()+
  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+
  annotation_custom(my_grob2)

globalall <- other %>% select(gene,glo.jt.h2,glo.jt.se,glo.jt.P,gloPlt05,ensid) %>% arrange(glo.jt.h2) %>% 
...
names(globalall) = c('gene','h2','se','jt.P','Plt05','ensid')
globalall <- globalall %>% mutate(ymin =  h2 - 2 * se, ymax =  h2 + 2 * se )
table(globalall$jt.P < 0.05,useNA='i')
sum(table(globalall$jt.P < 0.05,useNA='i'))
ptrue <-signif(table(globalall$jt.P < 0.05,useNA='i')/sum(table(globalall$jt.P < 0.05,useNA='i')),3)*100
ptrue
meanh2<-round(mean(globalall$h2,useNA="T"),3)
meanh2
meanse <- round(mean(globalall$se),3)
meanse
globalall <- mutate(globalall,P=factor(Plt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(globalall))

glolist <- dplyr::filter(globalall,Plt05==TRUE) %>% dplyr::select(gene)
table(glolist$gene %in% glopriorlist$gene)

my_grob2 = grobTree(textGrob(substitute(paste("% P < 0.05: ", m),list(m=ptrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

cother<-ggplot(globalall,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=P) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + 
  xlab(expression("genes ordered by distal h"^2))+theme_bw()+coord_cartesian(ylim=c(-1.05,1.55))+
  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+
  annotation_custom(my_grob2)
