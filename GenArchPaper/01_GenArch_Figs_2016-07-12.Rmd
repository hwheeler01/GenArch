...
fdrother <- mutate(fdrother, loc.jt.P=ifelse(is.na(loc.jt.P),1,loc.jt.P),glo.jt.P=ifelse(is.na(glo.jt.P),1,glo.jt.P)) %>% mutate(loc.jt.Q = p.adjust(loc.jt.P,method="BH"), glo.jt.Q = p.adjust(glo.jt.P,method="BH")) %>% mutate(locQlt05=loc.jt.Q < 0.05,gloQlt05=glo.jt.Q < 0.05) %>% mutate(`distal q-value`=factor(gloQlt05,labels=c('\u2265 0.05','< 0.05')))
table(fdrother$gloQlt05)

##Plot FDR based results
a<-ggplot(fdrother,aes(x=loc.jt.h2,y=glo.jt.h2,color=`distal q-value`)) + geom_point(cex=1,alpha=2/3)  + 
...
cigt0 <- local$ymin>0
table(local$jt.Q < 0.05,useNA='i')
sum(table(local$jt.Q < 0.05,useNA='i'))
qtrue <-round(table(local$jt.Q < 0.05,useNA='i')/sum(table(local$jt.Q < 0.05,useNA='i')),3)*100
qtrue
meanh2<-round(mean(local$h2),3)
meanh2
meanse <- round(mean(local$se),3)
meanse
local <- mutate(local,`q-value`=factor(Qlt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(local))
my_grob2 = grobTree(textGrob(substitute(paste("% q-value < 0.05: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

b<-ggplot(local,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`q-value`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by local h"^2))+theme_bw()+  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+ annotation_custom(my_grob2) 
...
table(global$jt.Q < 0.05,useNA='i')
sum(table(global$jt.Q < 0.05,useNA='i'))
qtrue <-signif(table(global$jt.Q < 0.05,useNA='i')/sum(table(global$jt.Q < 0.05,useNA='i')),3)*100
qtrue
meanh2<-round(mean(global$h2),3)
meanh2
meanse <- round(mean(global$se),3)
meanse
global <- mutate(global,`q-value`=factor(Qlt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(global))

glopriorlist <- dplyr::filter(global,Qlt05==TRUE) %>% dplyr::select(gene)

my_grob2 = grobTree(textGrob(substitute(paste("% q-value < 0.05: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

c<-ggplot(global,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`q-value`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + xlab(expression("genes ordered by distal h"^2))+theme_bw()+coord_cartesian(ylim=c(-1.05,1.55)) + theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+ annotation_custom(my_grob2)
...
other <- mutate(d, loc.jt.P = 2*pnorm(-abs(loc.jt.h2/loc.jt.se)) , glo.jt.P = 2*pnorm(-abs(glo.jt.h2/glo.jt.se))) %>% mutate(loc.jt.P=ifelse(is.na(loc.jt.P),1,loc.jt.P),glo.jt.P=ifelse(is.na(glo.jt.P),1,glo.jt.P)) %>% mutate(loc.jt.Q = p.adjust(loc.jt.P,method="BH"), glo.jt.Q = p.adjust(glo.jt.P,method="BH")) %>%  mutate(locQlt05=loc.jt.Q < 0.05,gloQlt05=glo.jt.Q < 0.05) %>% mutate(`distal q-value`=factor(gloQlt05,labels=c('\u2265 0.05','< 0.05')))
table(other$gloQlt05)

aother<-ggplot(other,aes(x=loc.jt.h2,y=glo.jt.h2,color=`distal q-value`)) + geom_point(cex=1,alpha=2/3) + xlab(expression("local h"^2)) + ylab(expression("distal h"^2)) + coord_cartesian(xlim=c(-0.05,1.05),ylim=c(-0.7,1.3)) + theme_bw() +  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(1,1),legend.position=c(1,1))
...
table(localall$jt.Q < 0.05,useNA='i')
sum(table(localall$jt.Q < 0.05,useNA='i'))
qtrue <-round(table(localall$jt.Q < 0.05,useNA='i')/sum(table(localall$jt.Q < 0.05,useNA='i')),3)*100
qtrue
meanh2<-round(mean(localall$h2),3)
meanh2
meanse <- round(mean(localall$se),3)
meanse
localall <- mutate(localall,`q-value`=factor(Qlt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(localall))
my_grob2 = grobTree(textGrob(substitute(paste("% q-value < 0.05: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))

bother<-ggplot(localall,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`q-value`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by local h"^2))+theme_bw()+ theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+  annotation_custom(my_grob2)
...
table(globalall$jt.Q < 0.05,useNA='i')
sum(table(globalall$jt.Q < 0.05,useNA='i'))
qtrue <-signif(table(globalall$jt.Q < 0.05,useNA='i')/sum(table(globalall$jt.Q < 0.05,useNA='i')),3)*100
qtrue
meanh2<-round(mean(globalall$h2,useNA="T"),3)
meanh2
meanse <- round(mean(globalall$se),3)
meanse
globalall <- mutate(globalall,`q-value`=factor(Qlt05,labels=c("\u2265 0.05","< 0.05")),position=1:nrow(globalall))
...
cother<-ggplot(globalall,aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`q-value`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + xlab(expression("genes ordered by distal h"^2))+theme_bw()+coord_cartesian(ylim=c(-1.05,1.55))+  theme(axis.text=element_text(size=16),axis.title=element_text(size=18,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+annotation_custom(my_grob2)
...
res<-summary(lm(log10(newdata$CrossTissue)~newdata$entropy))
pval<-data.frame(p=signif(res$coef[2,4],2))
b<-ggplot(newdata,aes(x=entropy,y=log10(CrossTissue)))+geom_point(alpha=1/4)+geom_smooth(method='lm')+theme_bw(16)+ylab(expression(paste("log"[10]," Cross-Tissue ", "h"^{2}))) + xlab("Uniformity") +geom_text(data=pval,aes(label=paste("P = ", p, sep="")), x=-Inf, y=Inf, hjust=-0.5, vjust=2, size=5)
b
