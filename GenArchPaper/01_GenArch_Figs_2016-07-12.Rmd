...
cigt0 <- local$ymin>0
table(local$jt.Q < 0.1)
sum(table(local$jt.Q < 0.1))
qtrue <-round(table(local$jt.Q < 0.1)/sum(table(local$jt.Q < 0.1)),3)*100
qtrue
...
table(global$jt.Q < 0.1)
sum(table(global$jt.Q < 0.1))
qtrue <-signif(table(global$jt.Q < 0.1)/sum(table(global$jt.Q < 0.1)),3)*100
qtrue
...
other <- mutate(d, loc.jt.P = 2*pnorm(-abs(loc.jt.h2/loc.jt.se)) , glo.jt.P = 2*pnorm(-abs(glo.jt.h2/glo.jt.se)))  %>% mutate(loc.jt.Q = p.adjust(loc.jt.P,method="BH"), glo.jt.Q = p.adjust(glo.jt.P,method="BH")) %>%  mutate(locQlt05=loc.jt.Q < 0.1,gloQlt05=glo.jt.Q < 0.1) %>% mutate(`distal FDR`=factor(gloQlt05,labels=c('\u2265 0.1','< 0.1')))
...
table(localall$jt.Q < 0.1)
sum(table(localall$jt.Q < 0.1))
qtrue <-round(table(localall$jt.Q < 0.1)/sum(table(localall$jt.Q < 0.1)),3)*100
qtrue
...
table(globalall$jt.Q < 0.1)
sum(table(globalall$jt.Q < 0.1))
qtrue <-signif(table(globalall$jt.Q < 0.1)/sum(table(globalall$jt.Q < 0.1)),3)*100
qtrue
...
dplot <- ggplot(groupdata,aes(h2,col=Uniformity,fill=Uniformity))+geom_density(alpha=0.2)+xlab(expression(paste("Cross-Tissue ", "h"^{2})))+theme_bw(16)+ theme(legend.position = c(0.8, 0.8))
ggplot(groupdata,aes(entropy,col=`cross-tissue h2`,fill=`cross-tissue h2`)) + geom_density(alpha=0.2)+xlab("Uniformity")+theme_bw(16)+ theme(legend.position = c(0.2, 0.8)) 
...
c<-ggplot(gentPPA, aes(x=geneorder,y=variable,fill=value)) + geom_tile() + ylab('Tissue PPA') + xlab('Representative genes sorted by Uniformity')+theme_bw(16)
c
