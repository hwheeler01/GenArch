...
fdrother <- mutate(fdrother, loc.jt.P=ifelse(is.na(loc.jt.P),1,loc.jt.P),glo.jt.P=ifelse(is.na(glo.jt.P),1,glo.jt.P)) %>% mutate(loc.jt.Q = p.adjust(loc.jt.P,method="BH"), glo.jt.Q = p.adjust(glo.jt.P,method="BH")) %>% mutate(locQlt05=loc.jt.Q < 0.1,gloQlt05=glo.jt.Q < 0.1) %>% mutate(`distal q-value`=factor(gloQlt05,labels=c('\u2265 0.1','< 0.1')))
...
cigt0 <- local$ymin>0
table(local$jt.Q < 0.1,useNA='i')
sum(table(local$jt.Q < 0.1,useNA='i'))
qtrue <-round(table(local$jt.Q < 0.1,useNA='i')/sum(table(local$jt.Q < 0.1,useNA='i')),3)*100
qtrue
meanh2<-round(mean(local$h2),3)
meanh2
meanse <- round(mean(local$se),3)
meanse
local <- mutate(local,`q-value`=factor(Qlt05,labels=c("\u2265 0.1","< 0.1")),position=1:nrow(local))
my_grob2 = grobTree(textGrob(substitute(paste("% q-value < 0.1: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))
...
table(global$jt.Q < 0.1,useNA='i')
sum(table(global$jt.Q < 0.1,useNA='i'))
qtrue <-signif(table(global$jt.Q < 0.1,useNA='i')/sum(table(global$jt.Q < 0.1,useNA='i')),3)*100
qtrue
meanh2<-round(mean(global$h2),3)
meanh2
meanse <- round(mean(global$se),3)
meanse
global <- mutate(global,`q-value`=factor(Qlt05,labels=c("\u2265 0.1","< 0.1")),position=1:nrow(global))
...
table(localall$jt.Q < 0.1,useNA='i')
sum(table(localall$jt.Q < 0.1,useNA='i'))
qtrue <-round(table(localall$jt.Q < 0.1,useNA='i')/sum(table(localall$jt.Q < 0.1,useNA='i')),3)*100
qtrue
meanh2<-round(mean(localall$h2),3)
meanh2
meanse <- round(mean(localall$se),3)
meanse
localall <- mutate(localall,`q-value`=factor(Qlt05,labels=c("\u2265 0.1","< 0.1")),position=1:nrow(localall))
my_grob2 = grobTree(textGrob(substitute(paste("% q-value < 0.1: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=14)))
...
table(globalall$jt.Q < 0.1,useNA='i')
sum(table(globalall$jt.Q < 0.1,useNA='i'))
qtrue <-signif(table(globalall$jt.Q < 0.1,useNA='i')/sum(table(globalall$jt.Q < 0.1,useNA='i')),3)*100
qtrue
meanh2<-round(mean(globalall$h2,useNA="T"),3)
meanh2
meanse <- round(mean(globalall$se),3)
meanse
globalall <- mutate(globalall,`q-value`=factor(Qlt05,labels=c("\u2265 0.1","< 0.1")),position=1:nrow(globalall))
