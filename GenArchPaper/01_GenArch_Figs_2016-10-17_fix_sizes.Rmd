---
title: "01_GenArch_Figs_2016-10-17_fix_sizes"
...
a<-ggplot(fdrother,aes(x=loc.jt.h2,y=glo.jt.h2,color=`distal FDR`)) + geom_point(cex=1,alpha=2/3)  + 
  ...
  theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),legend.justification=c(1,1),legend.position=c(1,1))
...
my_grob2 = grobTree(textGrob(substitute(paste("% FDR < 0.1: ", m),list(m=qtrue[2])), x=0.05,  y=0.70, hjust=0,gp=gpar(fontsize=12)))

b<-ggplot(local[complete.cases(local),],aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`FDR`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by local h"^2))+theme_bw(12)+  theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+ annotation_custom(my_grob2) 
...
c<-ggplot(global[complete.cases(global),],aes(x=position,y=h2,ymin=ymin, ymax=ymax, color=`FDR`) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + xlab(expression("genes ordered by distal h"^2))+theme_bw(12)+coord_cartesian(ylim=c(-1.05,1.55)) + theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),legend.justification=c(0,1),legend.position=c(0,1))+ annotation_custom(my_grob2)
...
bother<-ggplot(localall[complete.cases(localall),],aes(x=position,y=h2,ymin=ymin, ymax=ymax) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("local h"^2)) + xlab(expression("genes ordered by local h"^2))+theme_bw(12)+coord_cartesian(ylim=c(-1.05,2))
...
cother<-ggplot(globalall[complete.cases(globalall),],aes(x=position,y=h2,ymin=ymin, ymax=ymax) ) + geom_pointrange(col='gray')+geom_point()+ylab(expression("distal h"^2)) + xlab(expression("genes ordered by distal h"^2))+theme_bw(12)+coord_cartesian(ylim=c(-1.05,2))
...
a <- ggplot(data_long, aes(x = as.numeric(levels(variable))[variable] , y = value), group=gene) + geom_line(lwd=0.5,show_guide = FALSE,linetype=1) + aes(color = gene) + xlab(expression(paste("elastic net mixing parameter (",alpha, ")"))) + ylab(expression(paste("10-fold cross-validation R"^2))) + theme_bw(base_size = 12) + coord_cartesian(ylim=c(0.3,1),xlim=c(-0.02,1.02))+ geom_point(show_guide = FALSE)
print(a)
...
p<-ggplot(gdata, aes(y = `1` - R2, x = `1`, group=alpha, color=alpha)) + geom_point(show_guide = TRUE) + ylab(expression(paste("R"^2, " difference (LASSO - alpha)"))) + xlab(expression(paste("R"^2, " (LASSO)"))) +theme_bw(12)+ theme(legend.justification=c(0,1), legend.position=c(0,1))
...
setEPS()
postscript(file=fig.dir %&% "Fig-DGN-EN.eps",width=7.4,height=3.7)
multiplot(a+ggtitle('A\n')+theme(plot.title=element_text(hjust=0)),p+ggtitle('B\n')+theme(plot.title=element_text(hjust=0)),cols=2)
dev.off()
...
a<-ggplot(data,aes(x=pve50,y=pge50,ymin=pge025,ymax=pge975,col=LCS)) + geom_pointrange(col='gray') + geom_point() + theme_bw(12) + xlab("PVE") + ylab("PGE") + theme(legend.position = c(1,0),legend.justification = c(1,0))
...
p<-ggplot(tw,aes(x=h2,y=pve50,color=R2))+geom_point()+coord_cartesian(xlim=c(-0.05,1.1),ylim=c(-0.05,1.1))+xlab(expression("LMM h"^2))+ylab('BSLMM PVE')+geom_abline(intercept=0,slope=1) + scale_colour_gradient(low="pink",high="blue",name=expression("EN R"^2)) + facet_wrap(~tissue,ncol=3)+theme_bw(12)
...
p2<-p+geom_text(data=ann_text,aes(label=paste("EN_v_BSLMM:",R,sep=" ")),color="black",show_guide=F,parse=T,hjust=1,size=2.5)
p3<-p2+geom_text(data=ann_text2,aes(label=paste("EN_v_LMM:",R,sep=" ")),color="black",show_guide=F,parse=T,hjust=1,size=2.5)
setEPS()
postscript(file=fig.dir %&% "Fig-GTEx_TW_PVE_v_h2.eps",width=7.4,height=6.6)
p3
dev.off()
...
a <- p1 + theme_bw(12) #+ coord_cartesian(ylim=c(1.75,2.25))
print(a)
...
dplot <- ggplot(groupdata,aes(h2,col=Uniformity,fill=Uniformity))+geom_density(position="stack")+xlab(expression(paste("Cross-Tissue ", "h"^{2})))+theme_bw(12)+ theme(legend.position = c(0.7, 0.7))
ggplot(groupdata,aes(entropy,col=`cross-tissue h2`,fill=`cross-tissue h2`)) + geom_density(position = "stack")+xlab("Uniformity")+theme_bw(12)+ theme(legend.position = c(0.2, 0.8)) 
...
b<-ggplot(newdata,aes(x=entropy,y=CrossTissue))+geom_point(alpha=1/4)+geom_smooth(method='lm')+theme_bw(12)+ylab(expression(paste("Cross-Tissue ", "h"^{2}))) + xlab("Uniformity") +geom_text(data=pval,aes(label=paste("P = ", p, sep="")), x=-Inf, y=Inf, hjust=-0.5, vjust=2, size=5)
b
...
c<-ggplot(gentPPA, aes(x=geneorder,y=variable,fill=value)) + geom_tile() + ylab('Tissue PPA') + xlab('Representative genes sorted by Uniformity')+theme_bw(12) + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())
c
...
setEPS()
postscript(file=fig.dir %&% "Fig-CT-entropy-ppa.eps",width=7.5,height=3.5)
multiplot(c+ggtitle('A')+theme(plot.title=element_text(hjust=0)),dplot+ggtitle('B')+theme(plot.title=element_text(hjust=0)),cols=2)
dev.off()
```
...
setEPS()
postscript(file=fig.dir %&% "Fig-cor-StephensPr-v-PVE.eps",width=7.4,height=3.7)
par(mfrow=c(1,2))
corrplot(mat2.tw,method="circle",cl.lim = c(-0.35, 0.35),is.corr = FALSE,tl.cex=0.7,cl.cex=0.7,cl.ratio = 0.2,bg='black',tl.offset=0)
corrplot(mat2,method="circle",cl.lim = c(-0.35, 0.35),is.corr = FALSE,tl.cex=0.7,cl.cex=0.7,cl.ratio = 0.2,bg='black',tl.offset=0)
dev.off()
```