...
pops = scan(my.dir %&% 'poplist','c')
 for(alpha in c(0, 0.5,1)){ #rerun with 0 when CEU chr1 finishes
  for(pop in pops){
    data <- fread(my.dir %&% "GEUVADIS_" %&% pop %&% "_exp_10-foldCV_elasticNet_alpha" %&% alpha %&% "_1KGP_geuMAF0.01_all_chr1-22_2016-04-16.txt")
...
for(alpha in c(0,0.5,1)){ #rerun with 0 when CEU chr1 finishes
  ...
  topcomp <- filter(comp,diff>0.25)
  print(ggplot(topcomp,aes(pop,fill=pop)) + geom_bar() + ggtitle("Number of genes R2 diff > 0.25"))
  botcomp <- filter(comp,diff<(-0.25))
  print(ggplot(botcomp,aes(pop,fill=pop)) + geom_bar() + ggtitle("Number of genes R2 diff < -0.25"))
  rm("allR2")
}
```

### Compare E-N in GBR to other pops
```{r,fig.width=10,fig.height=10}
pops = scan(my.dir %&% 'poplist','c')
for(alpha in c(0,0.5,1)){ #rerun with 0 when CEU chr1 finishes
  for(pop in pops){
  ...
  comp <- gather(allR2,pop,R2,-GBR) %>% mutate(diff=GBR-R2)
  print(ggplot(comp, aes(pop,diff,fill=pop,color=pop)) +  ylab("GBR R2 - pop R2") + coord_cartesian(ylim=c(-1,1)) + 
      geom_point(position=position_jitterdodge(),size=1) +
      geom_boxplot(fill="white",outlier.colour = NA, position = position_dodge()) + ggtitle("E-N alpha=" %&% alpha))
  topcomp <- filter(comp,diff>0.25)
  print(ggplot(topcomp,aes(pop,fill=pop)) + geom_bar() + ggtitle("Number of genes R2 diff > 0.25"))
...
pops = scan(my.dir %&% 'poplist','c')
 for(alpha in c(0,0.5,1)){ #rerun with 0 when CEU chr1 finishes
  for(pop in pops){
    ...
    r2 =fread(my.dir %&% "GEUVADIS_" %&% pop %&% "_exp_10-foldCV_elasticNet_alpha" %&% alpha %&% "_1KGP_geuMAF0.01_all_chr1-22_2016-04-16.txt") %>% mutate(ensid=gene,R2=ifelse(is.na(R2),0,R2)) 
