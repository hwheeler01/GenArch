...
for(i in 1:length(genelist)){
  ...
  colnames(genect) <- c("ID","TISSUE","observed","simulated")
  ...
  colnames(genets) <- c("ID","TISSUE","observed","simulated")
  ...
  ann_text <- data.frame( observed = rep(0,10), simulated = rep(-3,10), R= rvec[1:10], TISSUE = factor(tislist), ID=rep(0.9,10))
  p<-ggplot(geneall,aes(x=observed,y=simulated)) + geom_point() + geom_smooth(method="lm") + facet_wrap(~TISSUE,ncol=5) + theme_bw() + ggtitle(gene %&% " - " %&% g)
  print(p+geom_text(data=ann_text,aes(label=paste("R==",R,sep="")),color="black",show_guide=F,parse=T,hjust=0,size=3))
}
```
...
ann_text <- data.frame( observed = rep(0,10), simulated = rep(-2.5,10), R= rvec[1:10], TISSUE = factor(tislist), ID=rep(0.9,10))
p<-ggplot(geneall,aes(x=observed,y=simulated)) + geom_point() + geom_smooth(method="lm") + facet_wrap(~TISSUE,ncol=5) + theme_bw() + ggtitle(gene %&% " - " %&% g)
p1<-p+geom_text(data=ann_text,aes(label=paste("R==",R,sep="")),color="black",show_guide=F,parse=T,hjust=0,size=5)
...
p2<-ggplot(sum1all,aes(tsgenecor)) + geom_freqpoly() + facet_wrap(~tissue,ncol=5) + theme_bw() + xlab('Gene correlation (simulated v. observed)')
p2
