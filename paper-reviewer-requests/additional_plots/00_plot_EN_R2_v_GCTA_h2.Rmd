...
twprice <- left_join(tw,price,by='gene')
p=ggplot(twprice[complete.cases(twprice),],aes(x=h2,y=pve50,color=h2bloodcis))+geom_point()+coord_cartesian(xlim=c(-0.05,1.05),ylim=c(-0.05,1.05))+xlab(expression("GCTA h"^2))+ylab('BSLMM PVE')+geom_abline(intercept=0,slope=1) + scale_colour_gradient(low="pink",high="blue",name=expression("Price et al. h"^2)) + facet_wrap(~tissue,ncol=3)+theme_bw()
p
cor.test(twprice$pve50,twprice$h2bloodcis)
cor.test(twprice$h2,twprice$h2bloodcis)
