...
ggplot(all,aes(x=local.h2,y=R2)) + geom_point(alpha=1/4) + theme_bw() + xlab("GCTA h2") + ylab("R2 (alpha=0.5)")+geom_abline(c(0,1),color='gray')
...
##plot R2 vs. h2
p<-ggplot(tw,aes(x=h2,y=R2))+geom_point(alpha=0.4)+coord_cartesian(xlim=c(-0.05,1.05),ylim=c(-0.05,1.05))+xlab(expression("GCTA h"^2))+ylab('R2 (alpha=0.5)')+geom_abline(c(0,1),color='gray') + facet_wrap(~tissue,ncol=3)+theme_bw()

ann_text <- data.frame( h2 = rep(0.75,9), R2 = rep(0.05,9), R= rvec2[1:9], tissue = factor(tislist), ensid=rep(0.9,9),gene=rep(0.9,9))
...
p2
png(filename=fig.dir %&% "Fig-GTEx_TW_EN0.5R2_v_h2.png",width=600,height=600)
p2
dev.off()
```
