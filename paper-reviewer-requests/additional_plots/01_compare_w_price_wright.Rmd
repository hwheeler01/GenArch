...
ggplot(all,aes(x=local.h2,y=gtexWB))+geom_point(alpha=1/4)+geom_smooth()+theme_bw()+coord_cartesian(ylim=c(-0.15,1.02))
res<-cor.test(all$local.h2,all$gtexWB,method='s')
CIrho(res$estimate,length(all$local.h2))
#dgn v. price blood
ggplot(all,aes(x=local.h2,y=h2bloodcis))+geom_point(alpha=1/4)+geom_smooth()+theme_bw()
res<-cor.test(all$local.h2,all$h2bloodcis,method='s')
CIrho(res$estimate,length(all$local.h2))
#dgn v. wright peripheral blood
ggplot(all,aes(x=local.h2,y=h2PB))+geom_point(alpha=1/4)+geom_smooth()+theme_bw()
res<-cor.test(all$local.h2,all$h2PB,method='s')
CIrho(res$estimate,length(all$local.h2))
