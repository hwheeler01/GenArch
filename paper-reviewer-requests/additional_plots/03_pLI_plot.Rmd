...
p<-ggplot(d %>% filter(!is.na(pLI) & !is.na(local.h2.x)),aes(x=pLI,fill= h2)) + geom_density(alpha=0.5)+theme_bw(12)+ scale_fill_discrete(name=expression("h"^2)) + theme(legend.position=c(0.85,0.85))
p
...
p<-ggplot(d %>% filter(!is.na(pLI)),aes(x=pLI,fill= enR2)) + geom_density(alpha=0.5)+theme_bw(12)+ scale_fill_discrete(name=expression("EN R"^2)) + theme(legend.position=c(0.85,0.85))
p
png(filename=fig.dir %&% "pLI-by-R2.png",width=480,height=480)
p
dev.off()
p
ggsave(fig.dir %&% "pLI-by-R2.eps",device=cairo_ps,width=3.8,height=3.8)
```